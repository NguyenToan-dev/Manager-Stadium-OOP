...
bool NormalBill::generateImage(const QString& outputJpgPath) {
    ...
    QString nhanVienLap = AuthManager::instance().getCurrentUser().firstName + AuthManager::instance().getCurrentUser().lastName;

    draw("Mã hoá đơn:", "HĐ" + QDateTime::currentDateTime().toString("yyMMdd-hhmmss"));
    draw("Nhân viên lập:", nhanVienLap);
    draw("Ngày lập:", QDateTime::currentDateTime().toString("dd/MM/yyyy hh:mm:ss AP"));
    y += 10;
    draw("Khách hàng:", customerName);
    ...
    y += 20;
    p.setFont(lblF);
    p.drawText(tx + 450, y, "Tổng tiền:");
    p.setFont(valF);
    p.drawText(tx + 600, y, QLocale("vi_VN").toString(sum) + " đ");

    QPixmap logo(":/image/logo.jpg");
    if (!logo.isNull()) {
