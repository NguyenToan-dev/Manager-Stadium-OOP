...
bool AuthManager::registerUser(const QString& username, const QString& password, int roleIndex, const QString& firstName, const QString& lastName) {
...
bool AuthManager::validate(const QString& username, const QString& password) {
    ...
    while (!in.atEnd()) {
        ...
        // username:salt:hash|role|first|last
        QStringList segments = line.split('|');
        if (segments.size() < 4) continue;

        QStringList credentials = segments[0].split(':');
        if (credentials.size() != 3) continue;

        if (credentials[0] == username) {
            QByteArray hashedInput = hashPassword(password, credentials[1]);
            if (hashedInput != credentials[2].toUtf8())
                return false;

            // ✅ Đúng mật khẩu -> lưu thông tin đăng nhập
            currentUser.username = credentials[0];
            currentUser.role = segments[1].toInt();
            currentUser.firstName = segments[2];
            currentUser.lastName = segments[3];
    ...
    return false; // không tìm thấy user hoặc sai mật khẩu
}
